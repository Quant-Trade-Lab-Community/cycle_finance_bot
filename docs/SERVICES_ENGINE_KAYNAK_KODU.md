...
> `services-engine/` — **Genel amaç:** HFT sistemi çevresindeki yardımcı servisler: fiyat besleyici (), indikatör hesaplama (calc-ind), pazar yapısı tespiti (detect-ms), canlı OHLCV üretimi (ohlcv-engine, stream-ohlcv), uyarı motoru (alert-service) ve yürütme konsolu (exec-console).
| Klasör / Dosya | Anlamı |
|---|---|
...
| `/` | Binance WS'ten mark/last/index/bid/ask çekip ring + HTTP sunan daemon |
| `/Cargo.toml` | paketi; proje_core + contracts + transport + axum |
| `/src/main.rs` | WS pump → parser → ring; premiumIndex REST; HTTP lastprice API |
...
### `stream-ohlcv/src/main.rs`
**Detaylı açıklama:** Canlı OHLCV mum akışı üreten servistir. `POST /api/stream` ile `{symbol, start_ms, interval_secs}` alır; aynı anahtar için zaten stream varsa mevcut meta'yı döner, yoksa yeni `Stream` task'ı başlatır. `run_stream` önce interval ≥60s ise Binance'ten geçmişi sayfalar (maks 200×1000) ve kapanan mumları ring'e yayınlar; sonra'ten `last_price` çekerek canlı mumu oluşturur/günceller, bucket değişince kapanan mumu `closed=1` ile ring'e basar. `StreamRingBuffer`'a push'lar `ring_lock` ile seri hale getirilir; durdurma `AtomicBool` stop + task join ile olur.
**Neden kullandık:**
- 1sn altı interval desteği için Binance geçmişi değil canlı fiyatlarından mum üretir
...
/// canlı'ten oluşturulur (Binance Futures geçmişi 1s altını desteklemez).
...
pub struct StreamMeta {
    ...
    pub published: u64,
    /// En son görülen fiyat (lastprice).
...
//!     → (:3004 /api/lastprice/{symbol}) anlık fiyatı düzenli çek
...
async fn run_stream(app: Arc<AppState>, stream: Arc<Stream>) {
    ...
    // 2) Canlı döngü: lastprice ile mumları güncelle/kapat.
