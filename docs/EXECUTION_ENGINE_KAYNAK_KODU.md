...
├── Cargo.toml
    ...
    ├── src/signer.rs
        ...
        ├── src/paper/account.rs
        ├── src/paper/actor.rs
        ├── src/paper/config.rs
        ├── src/paper/domain_event.rs
        ├── src/paper/mod.rs
        ├── src/paper/position.rs
        ├── src/paper/risk.rs
        ├── src/paper/snapshot.rs
        ├── src/risk/checks.rs
...
| `src/gateway.rs` | `EngineHandle`/`Gateway` trait'i; stratejilerin PAPER/LIVE farkını görmeden emir verdiği soyut yüzey |
...
| `src/paper/mod.rs` | Paper (simülasyon) motoru modül bildirimi |
| `src/paper/account.rs` | Paper hesap durumu: serbest/kilitli bakiye, marj kilitleme |
| `src/paper/actor.rs` | `PaperEngineActor`: mark fiyatı ile dolum, limit emir, likidasyon/funding |
| `src/paper/config.rs` | `PAPER_` önekli env tabanlı paper konfigürasyonu |
| `src/paper/domain_event.rs` | Event-sourcing domain olayları (`OrderFilled`, `Liquidation`...) |
| `src/paper/position.rs` | `PositionManager`: ONE_WAY/HEDGE pozisyon matematikleri |
| `src/paper/risk.rs` | `RiskManager`: drawdown, günlük kayıp, kaldıraç, likidasyon |
| `src/paper/snapshot.rs` | Paper okuma snapshot'ı (`PositionView`, `TradeView`) |
...
### `src/paper/mod.rs`
**Detaylı açıklama:** Paper (simülasyon) motorunun modül bildirimi. `config`, `account`, `actor`, `position`, `risk`, `domain_event`, `snapshot` alt modüllerini açıklar. Eski `db_writer` modülünün kaldırıldığını, kalıcılığın artık tek `DomainEvent` kanalından beslenen projection'larla (paper-service) yapıldığını not eder.
**Neden kullandık:**
- Paper motoru ayrı katman: canlı ve simülasyon kodu birbirine karışmaz.
- Event-sourcing: kalıcılık tek olay akışına bağlanır.

```mermaid
flowchart TD
    A["paper modülü"] --> B["actor: emir işleme"]
    A --> C["account: bakiye/marj"]
    A --> D["position: ONE_WAY/HEDGE"]
    A --> E["risk: drawdown/likidasyon"]
    A --> F["domain_event: olay akışı"]
    A --> G["snapshot: okuma görünümü"]
    F --> H["paper-service projection"]
```

### `src/paper/account.rs`
**Detaylı açıklama:** Paper hesap durumu; serbest ve kilitli bakiye haritaları. `lock_funds` yeterli serbest bakiye varsa kilitliye taşır (marj rezervi), `unlock_funds` kilitlenen kadarını geri verir, `deduct_locked_funds` komisyon/gerçekleşme düşer, `add_free_funds`/`deduct_free_funds` serbest bakiyeyi günceller. `subtract_free_funds_unchecked` short pozisyon borçlanmasında bakiyenin negatife düşmesine izin verir.
**Neden kullandık:**
- Serbest/kilitli ayrımı: açık marj görünmez, gerçek kullanılabilir bakiye net izlenir.
- Basit HashMap: yüksek güncelleme frekansı için yeterli performans.
- Kilit işlemlerine bakiye kontrolü: eksi bakiye yalnızca short'un kendi akışında oluşur.

```mermaid
flowchart TD
    A["lock_funds(asset, amt)"] --> B{"free >= amt?"}
    B -->|"hayır"| C["Err: yetersiz"]
    B -->|"evet"| D["free -= amt"]
    D --> E["locked += amt"]
    A2["unlock_funds"] --> F["min(locked, amt) → free"]
    A3["deduct_locked_funds"] --> G["min(locked, amt) düş"]
    A4["deduct_free_funds"] --> H{"free >= amt?"}
    H -->|"hayır"| I["Err"]
    H -->|"evet"| J["free -= amt"]
    A5["subtract_free_funds_unchecked"] --> K["free -= amt (short için)"]
```

### `src/paper/actor.rs`
**Detaylı açıklama:** `PaperEngineActor` emirleri mark fiyatı ile doldurur. `process_order` önce yapılandırılan gecikme/jitter'ı simüle eder, ardından `process_price_only`: mark fiyatı yoksa reddeder; HEDGE modda `BOTH` reddedilir. MARKET emirde risk kontrolü yapılır, marj kilitlenir (cross/isolated), dolum `apply_fill_dispatch` ile pozisyona işlenir, realized PnL ve komisyon bakiyeye yansır, `DomainEvent::OrderFilled` yayınlanır. LIMIT emirde marj kilitlenir; fiyat zaten geçmişse anında doldurulur, değilse `open_orders` listesine eklenir. `on_mark_tick` likidasyon, drawdown ve 8 saatlik funding'i yönetir; `check_limit_orders` her mark güncellemesinde bekleyen limitleri kontrol eder.
**Neden kullandık:**
- Event-sourcing: her dolum/likidasyon bir `DomainEvent` üretir, replay ile kurtarma mümkün.
- Modellenen gecikme: gerçek dünya latency'si simülasyon gerçekçiliği için taklit edilir.
- Cross/isolated marj mantığı: canlı borsa davranışıyla uyumlu.

```mermaid
flowchart TD
    A["ActorCommand al"] --> B{"SubmitOrder?"}
    B -->|"evet"| C["process_order: latency sim"]
    C --> D["process_price_only"]
    D --> E{"mark fiyatı var mı?"}
    E -->|"hayır"| F["MarketUnavailable"]
    E -->|"evet"| G{"HEDGE + positionSide BOTH?"}
    G -->|"evet"| H["RiskRejected"]
    G -->|"hayır"| I{"emir tipi?"}
    I -->|"MARKET"| J["risk check + marj kilitle"]
    J --> K["apply_fill_dispatch + realized PnL"]
    K --> L["emit OrderFilled"]
    I -->|"LIMIT"| M{"mark zaten crossed?"}
    M -->|"evet"| N["fill_limit"]
    M -->|"hayır"| O["open_orders'a ekle"]
    I -->|"diğer"| P["MarketUnavailable"]
    B -->|"MarkPriceUpdate"| Q["mark_prices güncelle"]
    Q --> R["on_mark_tick: likidasyon + funding"]
    Q --> S["check_limit_orders"]
    B -->|"SetPositionMode / SetMarginType"| T["mod kontrol + güncelle"]
```

### `src/paper/config.rs`
**Detaylı açıklama:** Paper motorunun `PAPER_` önekli env konfigürasyonu: başlangıç bakiyeleri (USDT/BTC), maker/taker komisyonu, temel gecikme ve jitter, DB yolu, kalıcılık ayarları (recover, WAL, batch interval), pozisyon modu ve marj tipi varsayılanları ile risk parametreleri (min notional, max leverage, max drawdown, max daily loss).
**Neden kullandık:**
- Ayrı önek (`PAPER_`): canlı `EXEC_` config'iyle karışmaz.
- Tüm değerler varsayılanlı: servis config olmadan da başlar.
- Risk parametreleri env'den: canlıya geçiş öncesi simülasyon gerçekçi kurulur.

```mermaid
flowchart TD
    A["PAPER_ env değişkenleri"] --> B["initial_usdt / initial_btc"]
    A --> C["maker_fee / taker_fee"]
    A --> D["base_latency_ms / jitter"]
    A --> E["db_path / wal / batch"]
    A --> F["position_mode / margin_type"]
    A --> G["min_notional / max_leverage / max_drawdown / max_daily_loss"]
    B --> H["PaperConfig"]
    C --> H
    F --> H
    G --> H
...
### `src/paper/domain_event.rs`
**Detaylı açıklama:** Paper sisteminin event-sourcing olayları. `OrderCreated`, `OrderFilled`, `OrderCancelled`, `PositionOpened`, `PositionClosed`, `Liquidation`, `FundingRateApplied` varyantları serde tag'li (snake_case) JSON'a eşlenir. `OrderFilled` net nakit etkisini (`cash_delta`) ve realized PnL'yi taşır; olaylar event store'a yazılır ve çökmede replay ile son duruma ulaşılır.
**Neden kullandık:**
- Serde tag'lı enum: olaylar JSON olarak saklanıp yeniden okunabilir.
- `cash_delta` ve `realized_pnl` ayrımı: projection'lar net etkiyi hesaplar.
- Tek olay akışı: kalıcılık ve replikasyon aynı kanaldan beslenir.

```mermaid
flowchart TD
    A["DomainEvent"] --> B["OrderCreated"]
    A --> C["OrderFilled: cash_delta + realized_pnl"]
    A --> D["OrderCancelled"]
    A --> E["PositionOpened / PositionClosed"]
    A --> F["Liquidation"]
    A --> G["FundingRateApplied"]
    C --> H["event store"]
    H --> I["replay → son durum"]
```

### `src/paper/position.rs`
**Detaylı açıklama:** Pozisyon yönetimi; boyut birimi USDT notional'dır (Long pozitif, Short negatif). `Position.unrealized_pnl` ve `notional` mark fiyatına göre hesaplanır; `liquidation_price` basitleştirilmiş cross-margin formülüyle bulunur. `PositionManager` ONE_WAY (sembol→pozisyon) ve HEDGE ((sembol, taraf)→pozisyon) olmak üzere iki harita tutar. `apply_fill` aynı yönde ağırlıklı ortalama giriş fiyatı, ters yönde kapanış/realized PnL ve yön değişimini yönetir; `apply_fill_hedge` tarafları ayrı ayrı izler.
**Neden kullandık:**
- USDT notional birimi: borsa davranışıyla uyumlu, PnL yüzde/notional tabanlı.
- İki mod tek yöneticide: ONE_WAY ve HEDGE aynı API ile yönetilir.
- Ağırlıklı ortalama giriş: kısmi artışlarda giriş fiyatı maliyeti yansıtır.

```mermaid
flowchart TD
    A["apply_fill(symbol, signed_qty)"] --> B{"mevcut pozisyon var mı?"}
    B -->|"yok"| C["yeni pozisyon aç"]
    B -->|"var"| D{"aynı yön mü?"}
    D -->|"evet"| E["ağırlıklı ortalama giriş"]
    D -->|"hayır"| F["kapat: realized PnL"]
    F --> G{"pozisyon sıfırlandı mı?"}
    G -->|"evet"| H["kaldır"]
    G -->|"hayır"| I{"yön değişti mi?"}
    I -->|"evet"| J["taraf + entry güncelle"]
    A2["apply_fill_hedge"] --> K["(sembol, taraf) ayrı izle"]
    A3["liquidation_price"] --> L["entry × (1 ± 1/lev) + maint"]
```

### `src/paper/risk.rs`
**Detaylı açıklama:** Paper risk yöneticisi. `check_order` emir öncesi risk durumu (drawdown/günlük kayıp ihlali), minimum pozisyon boyutu (6 USDT), marj gereksinimi (notional/lev ≤ nakit) ve maksimum kaldıracı denetler. `on_mark_tick` equity, tepe equity, drawdown ve günlük kaybı hesaplar; durumları `RiskStatus` olarak günceller ve per-pozisyon likidasyon eşiğini denetler — ihlal edilen sembolleri döndürür (actor bunları mark fiyatından kapatır).
**Neden kullandık:**
- Durum makinesi (`RiskStatus`): ihlal sonrası emir girişi durdurulur.
- Likidasyon eşikleri: gerçekçi zorunlu kapanış davranışı simüle edilir.
- Drawdown/günlük kayıp: strateji kısıtlamaları simülasyonda da uygulanır.

```mermaid
flowchart TD
    A["check_order(notional, lev, cash)"] --> B{"risk status durdurdu mu?"}
    B -->|"evet"| C["Err"]
    B -->|"hayır"| D{"notional < min?"}
    D -->|"evet"| E["Err"]
    D -->|"hayır"| F{"marj gereksinimi > cash?"}
    F -->|"evet"| G["Err"]
    F -->|"hayır"| H{"lev > max?"}
    H -->|"evet"| I["Err"]
    H -->|"hayır"| J["Ok"]
    A2["on_mark_tick"] --> K["equity + peak + drawdown + daily loss"]
    K --> L{"drawdown / daily loss ihlali?"}
    L -->|"evet"| M["RiskStatus güncelle"]
    L -->|"hayır"| N["RiskStatus::Ok"]
    K --> O{"pozisyon likidasyon eşiği?"}
    O -->|"aşıldı"| P["Liquidation + sembol listesi"]
```

### `src/paper/snapshot.rs`
**Detaylı açıklama:** Paper okuma snapshot'ı. `PaperSnapshot::build` pozisyon yöneticisindeki tüm pozisyonları `PositionView`'a dönüştürür: mark fiyatı, gerçekleşmemiş PnL, PnL yüzdesi, likidasyon fiyatı ve marj tipi hesaplar. `TradeView` son işlemleri taşır (en fazla 200). Actor her komut sonrası `publish_snapshot` ile `Arc<RwLock<PaperSnapshot>>`'ı günceller; API/CLI buradan okur.
**Neden kullandık:**
- Yazma actor'de sıralı, okuma kilitli snapshot'tan: yarış yok.
- `PositionView` sunum modeli: iç pozisyon modelini API'ye sızdırmaz.
- Son işlem geçmişi: operatör görünürlüğü için sınırlı tutulur.

```mermaid
flowchart TD
    A["publish_snapshot"] --> B["PaperSnapshot::build"]
    B --> C["positions.all() → PositionView"]
    C --> D["mark / unrealized pnl / pnl pct / liq price"]
    D --> E["PaperSnapshot yaz"]
    E --> F["Arc RwLock ile paylaş"]
    F --> G["API/CLI okuma"]
```
