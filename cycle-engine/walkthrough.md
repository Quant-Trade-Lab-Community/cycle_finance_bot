...
> - `engine/src/main.rs` (DATA terminali) kaldırıldı; kalıcılık artık **TimescaleDB**'dir (PC'ye native kurulum).
...
> - **`` servisi kaldırıldı**; tüketicileri (breakout, alert, stream-ohlcv, risk-worker) artık flow ring'lerini RAM'den okur. `ai-engine` de Desktop'a taşındı.
...
*   `engine`: TitaniumOrchestrator spin-loop ve catch_unwind izolasyonu (`orchestrator.rs`), state yönetimi (`state.rs`), CLI araçları (`correlation_cli.rs`, `strategy_cli.rs`) ve ana giriş noktası (`main.rs`)
*   `persistence`: TimescaleDB batch writer (`db.rs`) ve ClickHouse adaptörü (`clickhouse.rs`)
...
*   Workspace içerisindeki bağımlı servislerin (`alert-service`, ``, `breakout-strategy`, `calc-ind` vb.) `Cargo.toml` dosyaları ve kaynak kodlarındaki import yolları yeni katman crate'lerine yönlendirilerek güncellenmiştir.
