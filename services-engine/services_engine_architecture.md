...
- ``: Binance Futures WS'den fiyat çeker, ring'e yayar.
...
| **** | Binance Futures WS'den fiyat çeker, ring'e yayar | src/main.rs, pipeline.rs |
...
| `/cycle_finance_pricefeed` | 20_000 | main.rs:38,305 | alert-service source.rs:110; breakout main.rs:169 |
...
> **Note:** alert-service ayrıca `is_ring_alive()` ile ring'ini yoklar (source.rs:100-103).

### HTTP API Kaynakları

| Kaynak | Yöntem | Adres | Kod |
|:---|:---|:---|:---|
| | `GET /api/lastprice/{symbol}` | `:3004` | `context.rs:70-71` |
...
Binance Futures (WS fstream)
  └── (port 3004)
        ├── /cycle_finance_pricefeed ring → alert-service, breakout-strategy
...
| breakout-strategy | detect-ms (:3002) + (:3004) |
| exec-console | executiond (:3010) |
| stream-ohlcv | (:3004) |

### Ring Akışı

- ring'i en çok bağlanan ortak veri yoludur (3 servis okur).
- calc-ind ve stream-ohlcv ring'leri üretici/tüketici çiftidir (isteğe bağlı binary akış).

### DB tüketicileri

- strategies-engine risk_analysis.rs → TimescaleDB `trades` (market_data).
