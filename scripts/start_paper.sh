...
echo "=== Derleniyor... ==="
cargo build -p core -p paper-service
...
setsid env RUN_MODE=DATA ./target/debug/core > /tmp/data_terminal.log 2>&1 < /dev/null &
disown
...
setsid env \
  ...
  ./target/debug/paper-service > /tmp/paper_service.log 2>&1 < /dev/null &
disown
...
echo ""
echo "CLI örnekleri:"
echo "  ./target/debug/paper_cli --api http://$API_ADDR --user $ADMIN_USER --password $ADMIN_PASS status"
echo "  ./target/debug/paper_cli --api http://$API_ADDR --user $ADMIN_USER --password $ADMIN_PASS order --symbol BTCUSDT --side BUY --order-type MARKET --qty 0.001"
echo ""
